...
// Simulate an error or simulate something good just return state
let mockStore = () => {
...
describe('redux-reporter', () => {
   test('dispatch should return the action', () => {
       ...
       let action = {type:"normal", payload:{name:"Brian"}};
   ...
   test('logs errors properly without crashing the app', () => {
       ...
       let action = {type:"ERROR", payload:{name:"Brian"}};

       let result = store.dispatch(action);

       expect(result).toBeInstanceOf(Error);
       expect(result.action).toEqual(action);
       expect(result.message).toEqual('Error'); // We get our defined error back
